...
with open('../data/lignes.csv', 'r') as csv_file:
    ...
    for row in csv_reader:
        ...
        if row["ref:FR:STIF"] in opendata_lines:
            nb_ref_opendata = len(set(opendata_lines[row["ref:FR:STIF"]]))
        if row["ref:FR:STIF"] in osm_lines:
            nb_ref_osm = len(set(osm_lines[row["ref:FR:STIF"]]))
